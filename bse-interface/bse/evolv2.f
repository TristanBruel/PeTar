...
*
         if(kw.le.1.and.tm.lt.tphys.and..not.bss.and.
     &        bpp(jpmax+1,10).ne.6.0.and.bpp(jpmax+1,10).ne.3.0)then
            bss = .true.
            jp = MIN(jpmax,jp + 1)
            bpp(jp,1) = tphys
            bpp(jp,2) = mass(1)
...
*
 509  FORMAT(1P,E20.12,0P,2I4,1P,29E20.12,0P)
     ...
     &           (rad(2)**2.d0))**(1.d0/4.d0))
            write(10,509) tphys,kstar,mass0,mass,
     &           log10(lumin(1)),log10(lumin(2)),
     &           log10(rad(1)),log10(rad(2)),
     &           log10(teff1),log10(teff2),
     &           massc,radc,menv,renv,epoch,ospin,
     &           dmt-dmr,rad/rol,tb,sep,ecc
            tprint = tprint + dtp
      endif
