...
* by Disberg & Mandel 2025
          if(abskickflag.eq.5.and..not.ECSN_or_USSN)then
             call RandomLogNormal(disberg_mean,0.69d0,vk,idum,twopi)
             vk2 = vk*vk
...
* https://ui.adsabs.harvard.edu/abs/2020MNRAS.499.3214M/abstract
             ...
             call RandomTruncatedNormal(0, 0.3d0, idum,
     &                                  -1.d0, 10000.d0, vk)
             vk = mu_mm * (1 + vk)
             vk2 = vk * vk
          else
...
*       Default value for polar_kick_angle = 90.0
          ...
          sin_phi = (1.d0-bound)*ran3(idum) + bound
          phi = ASIN(sin_phi)
...
* companion and obliterates it
         ...
         phiE = ran3(idum) * twopi
         psiE = ran3(idum) * twopi
...
*
      ...
      END
***
      SUBROUTINE randomness3(idm,vx1,vy1,vz1,vx2,vy2,vz2)
*
      implicit none
*
      INTEGER idm
...
*
      pi = ACOS(-1.d0)
      twopi = 2.d0*pi
      alpha = twopi*ran3(idm) - pi
      gamma = twopi*ran3(idm) - pi
      beta = pi*ran3(idm) - (pi/2.d0)
...
* collide is whether the stars collide (logical)
      ...
      SUBROUTINE RandomLogNormal(mean, sigma, result, idum, twopi)
