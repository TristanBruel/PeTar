...
*
      ...
      integer nv,intpol,k
*     id_ev1: dummy star index for hrdiag calls within radius-step iterations
      integer id_ev1
      parameter(id_ev1=1)
      parameter(nv=50000)
*
      ...
      real*8 r,lum,mc,teff,rc,menv,renv,kick_info(2,19)
...
* PDK additions
      ...
      k = 1  !star index: always 1 in single-star evolv1 (used as sn counter in kick())
...
*
 ...
 20         dr = r - rm0
     ...
     &                               (tbgold - tmold)
               ...
               CALL star(kw,mass,mt,tm,tn,tscls,lums,GB,zpars)
               CALL hrdiag(mass,aj,mt,tm,tn,tscls,lums,GB,zpars,
     &                     r,lum,kw,mc,rc,menv,renv,k2,bhspin,id_ev1)
               goto 20
            endif
 30         continue
         endif
...
* AIC formation, will never happen here but...
     ...
     &                       (r**2.d0))**(1.d0/4.d0))
               ...
               CALL kick(kw,mass,mt,0.d0,0.d0,-1.d0,0.d0,0.d0,vk,k,
...
*
            ...
            mc1 = mc
            CALL hrdiag(mass,aj,mt,tm,tn,tscls,lums,GB,zpars,
     &                  r1,lum1,kw,mc1,rc1,menv1,renv1,k21,bhspin,
     &                  id_ev1)
            dr = r1 - rm0
            if(ABS(dr).gt.0.1d0*rm0)then
               dtm = dtr - ajhold*eps
