...
*       Convert Roche radius, radius and initial & current mass to SU.
      ...
      RC = RADC(I1)
      KW = KSTAR(I1)
      CALL star(KW,M0,M1,TM,TN,TSCLS,LUMS,GB,ZPARS, 0.d0, 0)
      CALL hrdiag(M0,AGE(I1),M1,TM,TN,TSCLS,LUMS,GB,ZPARS,
     &            RSJ,LUM,KW,MC,RC,MENV,RENV,K2, bhspin_dummy, 0)
*
...
*
     ...
     &        RHIGH,LUM,KW,MC,RC,MENV,RENV,K2, bhspin_dummy, 0)
...
* Find the overflow time using binary chopping.
   ...
   10       T = 0.5D0*(THIGH + TLOW)
     ...
     &           RR,LUM,KW,MC,RC,MENV,RENV,K2, bhspin_dummy, 0)
            IF(RR.LT.RL1)THEN
               TLOW = T
               RLOW = RR
            ELSE
